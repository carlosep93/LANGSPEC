...
class SequenceDecoder(object):
    def __init__(
        ...
        diverse_beam_groups=-1, diverse_beam_strength=0.5,layer=-1
    ):
    ...
    def generate_batched_itr(
        ...
        cuda=False, timer=None, prefix_size=0, max_length=220
    ):
        ...
        for sample in data_itr:
            ...
            if timer is not None:
                timer.start()
            with torch.no_grad():
                hypos,inner_states = self.generate(
                    encoder_input,
                    beam_size=beam_size,
            ...
            for i, id in enumerate(s['id'].data):
                ...
                yield id, src, ref, hypos[i],inner_states[i]
    ...
    def _generate(self, encoder_input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                inner_states.append(inner_state[0])
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores, inner_state = self._decode(tokens[:, :step + 1], encoder_outs, incremental_states)
    ...
    def _decode(self, tokens, encoder_outs, incremental_states):
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, attn, inner_state = self._decode_one(tokens, model, encoder_out, incremental_states, log_probs=True)
        ...
        return avg_probs, avg_attn, inner_state
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        with torch.no_grad():
            ...
            attn = decoder_out[1]
            inner_state = decoder_out[1]['inner_states'][self.layer]
