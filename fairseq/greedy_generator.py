...
class GreedyGenerator(object):
    def __init__(
        self,decoders, tgt_dict, beam_size=1, minlen=1, maxlen=None, stop_early=True,
        ...
        self.minlen = minlen
        max_decoder_len = min(d.max_positions() for d in self.decoders)
    ...
    def greedy_decode(self, encoder,encoder_out, src_tokens,bsz,beam_size=1, maxlen=None, prefix_tokens=None):
            ...
            if reorder_state is not None:
                ...
                for i, decoder in enumerate(self.decoders):
                    if isinstance(decoder, FairseqIncrementalDecoder):
                        decoder.reorder_incremental_state(incremental_states[decoder], reorder_state)
                    encoder_outs[i] = encoder.reorder_encoder_out(encoder_outs[i], reorder_state)
    ...
    def _decode(self, tokens, encoder_outs, incremental_states):
        if len(self.decoders) == 1:
            return self._decode_one(tokens, self.decoders[0], encoder_outs[0], incremental_states, log_probs=True)

        log_probs = []
        avg_attn = None
        for decoder, encoder_out in zip(self.decoders, encoder_outs):
            probs, attn = self._decode_one(tokens, decoder, encoder_out, incremental_states, log_probs=True)
            ...
            if attn is not None:
                if avg_attn is None:
                    ...
                    avg_attn.add_(attn)
        avg_probs = torch.logsumexp(torch.stack(log_probs, dim=0), dim=0) - math.log(len(self.decoders))
        if avg_attn is not None:
            avg_attn.div_(len(self.decoders))
