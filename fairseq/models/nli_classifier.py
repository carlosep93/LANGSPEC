...
class NliClassifierModel(BaseFairseqModel):
    """
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        self.max_reference_positions = args.max_reference_positions

        self.embed_tokens = embed_tokens
        self.embed_scale = math.sqrt(embed_dim)
        self.embed_positions = PositionalEmbedding(
            args.max_reference_positions, embed_dim, self.padding_idx,
            left_pad=left_pad,
    ...
    def forward(self, ref_tokens, ref_lengths):
        """
        Args:
            ref_tokens (LongTensor): tokens in the reference language of shape
                `(batch, ref_len)`
            ref_lengths (torch.LongTensor): lengths of each reference sentence of
                shape `(batch)`

        Returns:
            dict:
                - **encoder_out** (Tensor): the last encoder layer's output of
                  shape `(ref_len, batch, embed_dim)`
                - **encoder_padding_mask** (ByteTensor): the positions of
                  padding elements of shape `(batch, ref_len)`
        """
        # embed tokens and positions
        x = self.embed_scale * self.embed_tokens(ref_tokens)
        if self.embed_positions is not None:
            x += self.embed_positions(ref_tokens)
        ...
        encoder_padding_mask = ref_tokens.eq(self.padding_idx)
    ...
    def get_layer(self, ref_tokens, ref_lengths,nlayer=-1):
        """
        Args:
            ref_tokens (LongTensor): tokens in the reference language of shape
                `(batch, ref_len)`
    ...
    def max_positions(self):
        ...
        if self.embed_positions is None:
            return self.max_reference_positions
        return min(self.max_reference_positions, self.embed_positions.max_positions())
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        Args:
            ...
            encoder_padding_mask (ByteTensor): binary ByteTensor of shape
                `(batch, ref_len)` where padding elements are indicated by ``1``.

        Returns:
            encoded output of shape `(batch, ref_len, embed_dim)`
        """
        residual = x
...
@register_model_architecture('nli_classifier', 'nli')
