...
def get_generation_parser(interactive=False, default_task='translation'):
    ...
    add_interlingua_args(parser)
    if interactive:
        add_interactive_args(parser)
    return parser

def get_unsup_train_parser(default_task='translation'):
    parser = get_parser('Trainer', default_task)
    add_dataset_args(parser, train=True)
    add_distributed_training_args(parser)
    add_model_args(parser)
...
def get_generation_add_lang_parser(interactive=False, default_task='translation'):
    ...
    add_generation_args(parser)
    add_interlingua_args(parser)
    add_generate_add_lang_args(parser)
    if interactive:
        add_interactive_args(parser)
    return parser
...
def get_finetune_parser(default_task='translation'):
    parser = get_parser('Trainer', default_task)
    add_dataset_args(parser, train=True)
    add_distributed_training_args(parser)
    add_model_args(parser)
...
def add_interlingua_args(parser):
    group = parser.add_argument_group('Interlingua')
    group.add_argument('--all-langs',default='',type=str,help='All langs in checkpoint')
    return group
...
def add_generate_add_lang_args(parser):
    group = parser.add_argument_group('GenerateAddLang')
    group.add_argument('--enc-model', default='',type=str, help='Path to the model from which the encoder will be loaded')
    group.add_argument('--enc-key', default='en-en',type=str, help='Key in the model dictionary to load the encoder')
    group.add_argument('--dec-model', default='',type=str, help='Path to the model from which the decoder will be loaded')
...
def add_unsup_args(parser):
    ...
    group.add_argument('--pivot-prev-enc-model',default='',type=str, help='Path to previous model for pivot encoder languages')
    group.add_argument('--pivot-prev-dec-model',default='',type=str, help='Path to previous model for pivot decoder languages')
...
def add_model_args(parser):
    ...
    group.add_argument('--adv', default='False',metavar='BOOL',choices=['True','False'],help='Use or not adversarial training')
    group.add_argument('--n-dis', type=int, help='Number of updates of the discriminator for each update of the generators')
    return group
