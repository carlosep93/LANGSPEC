...
from fairseq.criterions.interlingua_loss import InterlinguaLabelSmoothedCrossEntropyCriterion




@register_task('dist_interlingua_translation')
class DistInterlinguaTranslationTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--distance', default='True', type=str, metavar='BOOL',
                            help='use distance measure')
    ...
    def __init__(self, args, dicts, training):
        ...
        self.distance = args.distance == 'True'
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False):
        ...
        for lang_pair in langs:
            if sample[lang_pair] is None or len(sample[lang_pair]) == 0:
                continue
            loss, sample_size, logging_output = criterion(model.models[lang_pair], encodings,sample[lang_pair],distance=self.distance)
            if ignore_grad:
                loss *= 0
            losses.append(loss)
