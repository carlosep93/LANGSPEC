...
from fairseq.options import add_lang_args
...
def main(args):
    ...
    encoder = SequenceDecoder(models, task.target_dictionary)

    if use_cuda:
        encoder.cuda()

    with progress_bar.build_progress_bar(args, itr) as t:
        encodings = encoder.generate_batched_itr(t, cuda=use_cuda, timer=gen_timer)
        data = {}
        i = 0
        for id,src,ref,hypos,inner_state in encodings:
            print('getting sentence', id)
            if i >= args.n_points:
                break
...
if __name__ == '__main__':
    parser = options.get_generation_add_lang_parser()
